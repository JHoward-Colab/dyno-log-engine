  ...
  for (var tempKey in tempFileMap) {
    ...
    if (pack.pvp && pack.intervals.length >= 3) {
      ...
      try {
        ...
        // Resolve DEV archive folder cleanly via Drive App
        var archiveFolder;
...
function retroactiveLogRecalculate() {
  ...
  // Build BASE_MODEL -> DYNAMIC_KEY map from Program_Registry
  ...
  if (regSheet) {
    ...
    for (var k = 1; k < regValues.length; k++) {
      ...
      if (bm) modelToDynamicKey[bm] = dk || pn || bm;
    }
  }
  ...
  for (var r = 1; r < logData.length; r++) {
    ...
    if (pName && logData[r][0] !== "") {
      ...
      var refRow = null;
      for (var mx = 1; mx < refData.length; mx++) {
        var mKey = String(refData[mx][mMap.dynamicKey] || "").trim().replace(/[-_\s]/g, "").toLowerCase();
    ...
    // Row array matches Master_Dyno_Log Cols S..V (Cols 19..22)
